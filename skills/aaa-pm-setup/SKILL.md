...
name: aaa-pm-setup
description: >-
  Scaffold docs/pm/ in any product repo (project-brief, intake, plan, jira epics/stories/tasks)
  and docs/qa/ACCESS.md. Complete project-brief until brief_status is complete.
metadata:
  pack: aaa-pm
...
## Step 0 — Scaffold `docs/pm/` + `docs/qa/`
...
| `templates/qa/ACCESS.template.md` | `docs/qa/ACCESS.md` |

Create `docs/qa/` if missing. **Do not** copy `templates/examples/` into the product repo.
...
> Brief complete. Next: **`/aaa-pm-plan`** → **`/aaa-pm-push`**. Feedback: **`/aaa-pm-intake`**. Ticket shapes: pack **`PROPOSAL.md`**.

## Rules

- No secrets or pricing tables in git-tracked PM docs.
- Epic/story/task bodies = Jira descriptions (no duplicate paste blocks).
