...
4. Per item:
   - **Small** → new task file in `docs/pm/jira/tasks/todo/` under active story (or new story if needed); set `jira_pushed: false` on jira OVERVIEW if new work. For multiple related tasks → **`/aaa-pm-task-add`**. Do not use intake to replace proper story split — large items → **`/aaa-pm-plan`**
