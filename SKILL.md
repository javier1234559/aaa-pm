...
- **Task** — **Done when** for Dev self-check; added when work starts (not auto-created at plan). Roll path/data into Story Handoff when story is ready.
- Only **`IN_PROGRESS`** epics get **story files** — see `templates/references/plan-story-first.md` and `epic-status.md`.
